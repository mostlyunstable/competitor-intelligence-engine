...
_KW: dict[str, list[str]] = {
    ...
    SEGMENT_PLANS: [
        "plan", "our plans", "choose your plan", "compare plans",
        "plan comparison", "plan options", "pricing plans",
        "subscription plans", "membership plans",
    ],
    SEGMENT_MEMBERSHIP: [
        "membership", "become a member", "member benefits",
        "join", "sign up", "register", "create account",
        "member area", "premium", "pro plan", "business plan",
    ],
    SEGMENT_ABOUT: [
        "about",
        "our story",
    ...
    SEGMENT_REVIEWS: [
        "review", "testimonial", "rating", "what our customers say",
        "customer story", "success story", "trustpilot", "g2",
        "capterra", "client feedback", "customer review",
        "star", "rated", "recommended",
    ],
    SEGMENT_FAQ: [
        "faq",
        "frequently asked",
    ...
    SEGMENT_CITIES: [
        "city", "cities", "service area", "location", "where we serve",
        "coverage area", "regions", "states", "zip code", "postal code",
        "available in", "nationwide", "local",
    ],
    SEGMENT_COVERAGE: [
        "coverage", "service area", "network", "supported region",
        "availability", "operating area", "footprint", "reach",
    ],
    SEGMENT_FEATURES: [
        "feature", "capability", "specification", "what's included",
        "includes", "included", "comparison", "compare", "vs",
        "feature matrix", "feature list",
    ],
    SEGMENT_LEGAL: [
        "legal", "privacy", "terms", "policy", "cookie policy",
        "gdpr", "ccpa", "disclaimer", "license", "agreement",
    ],
    SEGMENT_GALLERY: [
        "gallery", "portfolio", "showcase", "our work", "projects",
        "case study", "screenshot", "photo", "image gallery",
...
class PageSegmenter:
    ...
    def _collect_candidates(self, soup: BeautifulSoup) -> list[tuple[Tag, int]]:
        ...
        def _add(el: Tag, depth: int) -> None:
            ...
            if el.name not in ("header", "nav", "main", "footer", "aside") and len(text) < self.MIN_TEXT_LEN:
                return
            seen.add(eid)
